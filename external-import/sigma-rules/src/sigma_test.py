...
#                )
    ...
    def run_test(self):
        timestamp = int(time.time())
        now = datetime.utcfromtimestamp(timestamp)
        self.extract_rule_test("Sigma",now)
        
    def extract_rule_test(self,source,date):  
        
        
        friendly_name = "SIGMA-RULES run @ " + date.strftime("%Y-%m-%d %H:%M:%S")
        work_id = self.helper.api.work.initiate_work(
            self.helper.connect_id, friendly_name
        )
#        with zipfile.ZipFile(os.path.dirname(os.path.abspath(__file__)) + "/data.zip", 'r') as zip_ref:
...
#        if(source == 'Sigma'):
#            walk_dir=os.path.dirname(os.path.abspath(__file__))+ "/sigma-master/rules/"
        walk_dir="/data/osint/sigma/sigma-master/rules/"
        bundle_objects = []
        object_refs=[]
...
if __name__ == "__main__":
    try:
        ...
        sigma_rules_connector.run_test()
